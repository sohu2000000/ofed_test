...
function rep_intf_get() {
	...
	# Process devlink output to get SF ports
	while IFS= read -r line; do
		# Start of a new port entry
		if [[ $line =~ ^pci/ ]]; then
			# Reset flags and variables for new port
		...
		# Look for SF ports (flavour pcisf)
		if [[ $line =~ flavour\ pcisf ]]; then
			is_sf_port=true
			# Get the port name from the same line
			if [[ $line =~ netdev\ ([^[:space:]]+) ]]; then
				current_name="${BASH_REMATCH[1]}"

				# Get SF number from the same line
				if [[ $line =~ sfnum\ ([0-9]+) ]]; then
					current_sfnum="${BASH_REMATCH[1]}"
				fi

				# Get SF index from the port number in devlink output
				if [[ $line =~ pci/[^/]+/([0-9]+): ]]; then
