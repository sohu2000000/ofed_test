   ...
   #  fi
    ...
    # sleep 10
     ...
     for ((mdev=0; mdev<$mdevs ; mdev++)) ; do
	   ...
	   echo "devlink port add pci/$pci_device flavour pcisf pfnum $port_number sfnum $mdev"
	   result=$(devlink port add pci/$pci_device flavour pcisf pfnum $port_number sfnum $mdev)
	   ...
	   if [ $((mdev % 20)) -eq 0 ]; then
	   	sleep 1
	   fi


	   found_sf=0
	   while [ $found_sf -eq 0 ];do
